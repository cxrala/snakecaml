...
let%expect_test "Exercise 03a" =
  ...
  [%expect {| |}];
  test (0, 14) true;
  [%expect {| in_bounds for ((height 10) (width 15)) ((col 14) (row 0)) returned false unexpectedly |}];
  test (0, 15) false;
  [%expect {| |}];
  test (9, 0) true;
  [%expect {| |}];
  ...
  [%expect {| |}];
  test (9, 14) true;
  [%expect {| in_bounds for ((height 10) (width 15)) ((col 14) (row 9)) returned false unexpectedly |}];
  test (9, 15) false;
  [%expect {| |}];
  test (10, 14) false;
  [%expect {| |}]
