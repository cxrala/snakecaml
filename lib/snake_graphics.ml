...
module Colors = struct
  let black = Graphics.rgb 222 165 164
  let green = Graphics.rgb 202 231 193
  let head_color = Graphics.rgb 177 156 217
  let red = Graphics.rgb 255 105 97
  let blue = Graphics.rgb 174 198 207
  let gold = Graphics.rgb 255 255 000
...
let draw_header ~game_state ~score ~score2 =
  ...
  Graphics.moveto (play_area_width / 2) (play_area_height + 25);
  Graphics.draw_string (Printf.sprintf "Score 1: %d | Score 2: %d" score score2)
;;
...
let draw_snake snake_head snake_tail color =
  List.iter snake_tail ~f:(draw_block ~color);
...
let render game =
  ...
  draw_play_area ();
  draw_apple apple;
  draw_snake (Snake.head snake) (Snake.tail snake) Colors.blue;
  draw_snake (Snake.head snake2) (Snake.tail snake2) Colors.green;
